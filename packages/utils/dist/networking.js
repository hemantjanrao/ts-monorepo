...
async function getJSON(input, init) {
    try {
        ...
        throw new Error(error_1.stringifyError(`Networking/getJSON: An error was encountered while fetching ${JSON.stringify(input)}`, err));
    }
}
/**
...
async function apiCall(path, init) {
    ...
    try {
        ...
        if (err instanceof http_error_1.default)
            throw err;
        throw new Error(error_1.stringifyError(`Networking/apiCall: An error was encountered while making api call to ${path}`, err));
    }
    if (!response.ok)
